...
possibleFixationIdx = ETparams.data.vel < Scalers.peakDetectionThreshold;
...
for k = 1:max(fixLabeled)
    ...
    fNoise = ETparams.data.vel(floor(fixIdx(1)+centralFixSamples):ceil(fixIdx(end)-centralFixSamples));
...
ETparams.ModeNoise = ModeNoise;
ETparams.Percentile75=prctile(fixNoise,75);
ETparams.NoiseIQR=iqr(fixNoise);
% ETparams.data.peakDetectionThreshold        = Scalers.PeakVelocityThreshold;
% ETparams.data.saccadeVelocityThreshold      = Scalers.SacOnOffThresh;
% ETparams.data.WeakGlissadeVelocityThreshold = Scalers.WeakGlissadeVelocityThreshold;
ThisFileName=strcat(FileName(1:12),'_Noise.csv');
fprintf('FileName = %s\n',ThisFileName)
out=[ETparams.ModeNoise Scalers.PeakVelocityThreshold Scalers.SacOnOffThresh Scalers.WeakGlissadeVelocityThreshold];
...
ETparams.data.ModeNoise = ModeNoise;
...
ETparams.data.velPeakIdx  = ETparams.data.vel > Scalers.PeakVelocityThreshold;
